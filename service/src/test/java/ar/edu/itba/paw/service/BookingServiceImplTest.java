...
/*    @Test
    public void testAddBook() {
        ...
        when(mockBookingService.addBook(testRest.getId(), testUser, "2", new Date(), "12:00", null)).thenReturn(new ModelAndView("redirect:/user_profile/bookings"));
        when(mockBookingService.addBook(4, testUser, "2", new Date(), "12:00", null)).thenReturn(new ModelAndView("redirect:/404"));
    }

    @Test
    public void testIsAvailable() {
        when(mockRestaurantDao.register(REMAIL, RPASS, PHONE, NAME, ADDRESS, PRICE, CUISINE, NEIGHBORHOOD, false, CAPACITY)).thenReturn(testRest);
        when(mockBookingService.isAvailable(testRest,"12:00", new Date(),6)).thenReturn(false);
        when(mockBookingService.isAvailable(testRest,"12:00", new Date(),3)).thenReturn(true);
    }
    ...
    public void testDeleteBook(){
        ...
        when(mockBookingService.deleteBook(testUser, null, 1, null)).thenReturn(new ModelAndView("redirect:/user_profile/bookings"));
        when(mockBookingService.deleteBook(null, testRest, 1, null)).thenReturn(new ModelAndView("redirect:/restaurant_profile/bookings"));
        when(mockBookingService.deleteBook(null, null, 1, null)).thenReturn(new ModelAndView("redirect:/login"));
    }*/
}